...
export default function TopProdutosMulheres(props: TopProdutosMulheresProps) {
    ...
    const fetchProdutos = async () => {
        try {
            const response = await fetch('http://localhost:3001/listarProdutosMaisConsumidosMulheres');
            if (response.ok) {
    ...
    return (
        <div className="collection">
            <h2> Lista dos produtos mais consumidos por mulheres </h2>
            <button
            ...
            {produtos.map((produto, index) => (
                <div key={index} className="collection-item">
                    ...
                    Quantidade comprada por mulheres: {produto.quantidade_vendas_feminino}<br />
                    Quantidade vendida total: {produto.quantidade_vendas} <br />
                    Quantidade comprada por homens: {produto.quantidade_vendas_masculino} 
