...
interface TopProdutosHomensProps {
    seletorView: (valor: string, e: React.MouseEvent<HTMLButtonElement>) => void;
}

export default function TopProdutosHomens(props: TopProdutosHomensProps) {
    const [produtos, setProdutos] = useState<Produto[]>([]);


    const fetchProdutos = async () => {
        try {
            const response = await fetch('http://localhost:3001/listarProdutosMaisConsumidosHomens');
            if (response.ok) {
    ...
    return (
        <div className="collection">
            <h2> Lista dos produtos mais consumidos por homens </h2>
            <button
